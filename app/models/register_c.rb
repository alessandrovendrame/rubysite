class RegisterC < ApplicationRecord
  
  NAME = "Registro Dog. #{Date.today.year - 2000}"
  ...
  class << self
	...
	types.each do |t|
	  ...
	  RegisterC.define_method("assign_#{t}") do
	    unless send("new_#{t}_name").blank?
	      check_vehicle = Vehicle.find_by_name(send("new_#{t}_name"))
	      if check_vehicle
	        eval %(check_vehicle.#{t} = true)
	        check_vehicle.register_c = true # <<<<<<<<<< register
	        check_vehicle.save
	      else
	        check_vehicle = Vehicle.new(:name => send("new_#{t}_name"), t.to_sym => true, :register_c => true)
	        check_vehicle.save
	      end
